...
module.exports = function(id, sid, config, UTILS, projectileManager,
    ...
    this.updateInventory = function(itemId) {
        var invItem = items.list[itemId];
        if (!invItem || !invItem.group) return;
        var g = invItem.group.id;
        var cur = this.globalInventory[g];
        if (cur == undefined || (invItem.age || 0) > (items.list[cur].age || 0)) this.globalInventory[g] = itemId;
    };
    ...
    this.spawn = function(moofoll) {
        ...
        this.shootCount = 0;
        this.weaponXP = [];
        this.reloads = [{ current: 0, max: 0, previous: 0 }, { current: 0, max: 0, previous: 0 }, { current: 23, max: 23, previous: 23 }];
        this.bullTick = 0;
        this.globalInventory = {};
        this.poisonCount = 0;
    };
    ...
    this.isBullTickTime = function(adjust) {
        return (config.tick - this.bullTick - (adjust || 0)) % 9 === 0;
    };
    ...
    this.canBuild = function(item) {
        var tmpLimit = config.inSandbox ? (item.group.sandboxLimit || Math.max(item.group.limit * 3, 99)) : item.group.limit;
        if (tmpLimit && this.itemCounts[item.group.id] >= tmpLimit)
            return false;
