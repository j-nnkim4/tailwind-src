...
module.exports = function (GameObject, gameObjects, liveObjects, UTILS, config, players, server) {
    ...
    this.disableObj = function(obj) {
        obj.active = false;
        var tmpLive = liveObjects.indexOf(obj);
        if (tmpLive >= 0) liveObjects.splice(tmpLive, 1);
        if (server) {
    ...
    this.nearObjects = [];
    this.nearObjectCount = 0;
    this.buildNearObjects = function(player) {
        this.nearObjectCount = 0;
        var px = player.x2, py = player.y2;
        for (var i = 0; i < gameObjects.length; ++i) {
            var o = gameObjects[i];
            if (!o.active) continue;
            var dx = o.x - px, dy = o.y - py;
            if (dx * dx + dy * dy <= 2560000) this.nearObjects[this.nearObjectCount++] = o;
        }
    };
    ...
    this.add = function(sid, x, y, dir, s, type, data, setSID, owner) {
        ...
        for (var i = 0; i < gameObjects.length; ++i) {
            if (gameObjects[i].sid == sid) {
                ...
                if (tmpObj.active) keepHealth = tmpObj.health;
                break;
            }
        } if (!tmpObj) {
            for (var i = 0; i < gameObjects.length; ++i) {
            ...
            liveObjects.push(tmpObj);
        }
        if (setSID)
            tmpObj.sid = sid;
        tmpObj.init(x, y, dir, s, type, data, owner);
        if (keepHealth != undefined) tmpObj.health = keepHealth;
        if (server) {
